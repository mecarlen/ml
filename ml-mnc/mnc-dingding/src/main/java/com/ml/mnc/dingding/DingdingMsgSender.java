	...
	public void send(final DingdingMsgVO msg, long beginTimeMills) {
		...
		call.enqueue(new Callback() {
			...
			public void onResponse(Call call, Response response) throws IOException {
				...
				log.warn("sendDingdingFailure,message:{},url:{}", errMsg.toString(), req.url().toString());
				dingdingMsgService.sendFailure(msg, beginTimeMills, errMsg.toString());
			}

			@Override
			public void onFailure(Call call, IOException ex) {
				log.error("sendDingdingIOException,url:{}", req.url().toString(), ex);
